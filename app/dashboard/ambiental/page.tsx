  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <div className="mb-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {[
          { label: 'NÃO SE APLICA', count: processes.filter(p => (p.statusInscricaoAmbiental || p.status) === 'NÃO SE APLICA').length, color: 'border-gray-500' },
          { label: 'NÃO INICIADOS', count: processes.filter(p => (p.statusInscricaoAmbiental || p.status) === 'NÃO INICIADO').length, color: 'border-yellow-500' },
          { label: 'EM ANDAMENTO', count: processes.filter(p => EM_ANDAMENTO_STATUSES.includes(p.statusInscricaoAmbiental || p.status)).length, color: 'border-blue-500' },
          { label: 'APROVADOS', count: processes.filter(p => (p.statusInscricaoAmbiental || p.status) === 'APROVADO').length, color: 'border-green-500' },
          { label: 'CANCELADOS', count: processes.filter(p => (p.statusInscricaoAmbiental || p.status) === 'CANCELADO' || (p.statusInscricaoAmbiental || p.status) === 'REPROVADO').length, color: 'border-red-500' },
        ].map((card) => (
