  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <div className="mb-8 grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'NÃO INICIADOS', count: [...new Set(processes.filter(p => (p.statusInscricaoAnuencia || p.status) === 'NÃO INICIADO').map(p => p.inscricao))].length, color: 'border-yellow-500' },
          { label: 'EM ANDAMENTO', count: [...new Set(processes.filter(p => EM_ANDAMENTO_STATUSES.includes(p.statusInscricaoAnuencia || p.status)).map(p => p.inscricao))].length, color: 'border-blue-500' },
          { label: 'APROVADOS', count: [...new Set(processes.filter(p => (p.statusInscricaoAnuencia || p.status) === 'APROVADO').map(p => p.inscricao))].length, color: 'border-green-500' },
          { label: 'CANCELADOS', count: [...new Set(processes.filter(p => (p.statusInscricaoAnuencia || p.status) === 'CANCELADO' || (p.statusInscricaoAnuencia || p.status) === 'REPROVADO').map(p => p.inscricao))].length, color: 'border-red-500' },
        ].map((card) => (
