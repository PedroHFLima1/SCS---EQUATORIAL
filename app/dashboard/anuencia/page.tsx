...
export default function AnuenciaPage() {
  ...
  const canAccess = role === 'ADMIN' || role === 'GESTOR_ANUENCIA' || role === 'PARCEIRA';
  ...
  const filteredProcesses = useMemo(() => {
    ...
    if (statusProjetoFilter !== 'Todas as Fases') {
      result = result.filter(p => {
        const projStatus = p.statusAnuencia || p.status;
    ...
    return (
      <div className="p-8 flex flex-col items-center justify-center h-full">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Você não tem permissão para acessar o Módulo Anuência.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">MÓDULO ANUÊNCIA</h1>
      ...
      <div className="mb-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {[
          { label: 'NÃO SE APLICA', count: [...new Set(processes.filter(p => (p.statusInscricaoAnuencia || p.status) === 'NÃO SE APLICA').map(p => p.idSolicitacao || p.inscricao))].length, color: 'border-gray-500' },
          { label: 'NÃO INICIADOS', count: [...new Set(processes.filter(p => (p.statusInscricaoAnuencia || p.status) === 'NÃO INICIADO').map(p => p.idSolicitacao || p.inscricao))].length, color: 'border-yellow-500' },
          { label: 'EM ANDAMENTO', count: [...new Set(processes.filter(p => EM_ANDAMENTO_STATUSES.includes(p.statusInscricaoAnuencia || p.status)).map(p => p.idSolicitacao || p.inscricao))].length, color: 'border-blue-500' },
          { label: 'APROVADOS', count: [...new Set(processes.filter(p => (p.statusInscricaoAnuencia || p.status) === 'APROVADO').map(p => p.idSolicitacao || p.inscricao))].length, color: 'border-green-500' },
          { label: 'CANCELADOS', count: [...new Set(processes.filter(p => (p.statusInscricaoAnuencia || p.status) === 'CANCELADO' || (p.statusInscricaoAnuencia || p.status) === 'REPROVADO').map(p => p.idSolicitacao || p.inscricao))].length, color: 'border-red-500' },
        ].map((card) => (
